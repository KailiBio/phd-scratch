...
# -- Kaili
# This script is for making volcano plot for sanity check.
...
get_sanityCheck_matrix(){
    ...
    do
        ...
        mv tmp.matrix5.txt ./sanityCheck_matrix/${sample1}_VS_${sample2}_matrix.txt
        #
        #Rscript ${scriptDir}make_sanityCheck_volcano.R tmp.matrix5.txt ${sample1}_VS_${sample2}
        # # merge all figures
        # if [ -f RNAseq_DE_sanityCheck_volcano.pdf ]; then
        #     pdfjam --outfile tmp.pdf RNAseq_DE_sanityCheck_volcano.pdf ./volcano_check/${sample1}_VS_${sample2}.pdf
        #     mv tmp.pdf RNAseq_DE_sanityCheck_volcano.pdf
        # else
        #     cp ./volcano_check/${sample1}_VS_${sample2}.pdf RNAseq_DE_sanityCheck_volcano.pdf
        # fi
    done
}
...
#
get_sanityCheck_matrix ${outfile} ${DEG_folder}


# 2. make figures
get_sanityCheck_figs(){
    outfile=$1
    DEG_folder=$2
    #
    if [ -f ${outfile} ];then rm ${outfile}; fi
...
DEG_folder="de_M4_read"
#
get_sanityCheck_figs ${outfile} ${DEG_folder}


# 3. try different pseudo-count
## not working
