...
do
    ...
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$1]=$5}else{print $0,a[$2]}}' /data/projects/encode/data/${expID}/${tsvID}.tsv mm10_M4_M18_overlap_genelist2.txt > tmp_readCounts1.txt
    # spikeIn96, M18
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$1]=$5}else{print $0,a[$3]}}' ./rna_exp_Jul25/${expID}/${fileIDs}/${fileIDs}.genes.results tmp_readCounts1.txt >tmp_readCounts2.txt
    # make scatter
    cut -f 1,4,5 tmp_readCounts2.txt > tmp_readCounts.txt
    Rscript ${scriptDir}make_rnaseq_check_scatter.R tmp.txt ${sample_rep}_readCounts
done < mouse_RNA_fastq_list_merged_SampleInfo.txt

rm tmp_readCounts*.txt
