...
########
input_file = paste("./sanityCheck_matrix/",sample,"_matrix.txt",sep="")
...
p3 = ggplot(dat, aes(x=log2FoldChange, y=-log10(padj), col = sig)) + geom_point(alpha=0.5) +
  theme_minimal() + theme(legend.position = "none") +
  labs(title="Junko's DE call") + coord_cartesian(xlim=c(-20,20), ylim=c(0,300))


p4 = ggplot(dat, aes(x=my_log2FoldChange, y=-log10(my_padj), col=sig)) + geom_point(alpha=0.5) +
  theme_minimal() + theme(legend.position = "none") +
  labs(title="My DE call") + coord_cartesian(xlim=c(-20,20), ylim=c(0,300))


p = grid.arrange(p1, p2, p3, p4, ncol=2, top = sample)
p

out_file1 = paste("./DE_sanityCheck_fig/", sample, "_readCounts.pdf", sep="")
out_file2 = paste("./DE_sanityCheck_fig/", sample, "_readCounts.png", sep="")
