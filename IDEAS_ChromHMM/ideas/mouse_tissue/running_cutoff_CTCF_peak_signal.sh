...
do
    ...
    /data/zusers/fankaili/ideas/CTCF_impute/ctcf_samples/ctcf_samples_result/ctcf_samples.chr1.state >> mm10_ctcf_state.bed
done
...
do
    ...
    echo -e ${i}"\t"${a} >> mm10_ctcf_peak_cutoff_with_ctcf_motif_num.txt
    # high CTCF peaks in CTCF states
    b=`intersectBed -a tmp_peak.bed -b mm10_ctcf_state_sorted.bed -wa | sort -u | wc -l`
    echo -e ${i}"\t"${b} >>  mm10_ctcf_peak_cutoff_with_ctcf_state_num.txt
done
