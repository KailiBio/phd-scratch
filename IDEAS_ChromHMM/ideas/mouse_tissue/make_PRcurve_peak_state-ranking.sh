...
do
    ...
    echo $sample
    #
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$4]=$5}else{print $4,a[$4]}}' ./state_bed_${prefix}/${sample}_state_sorted.bed ./state_ranked_PR/${sample}_CTCFpeak_pos.txt > tmp.${prefix}_pos.txt
    ...
    #
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$4]=$5}else{print $4,a[$4]}}' ./state_bed_${prefix}/${sample}_state_sorted.bed ./state_ranked_PR/${sample}_CTCFpeak_neg.txt > tmp.${prefix}_neg.txt
