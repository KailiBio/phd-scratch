...
# -- Kaili
# This script is for getting CTCF signal of dhs-CTCF-imputed bins in given state.

# INPUT: state number
# OUTPUT: CTCF signal of all bins in that given state
# EXP: bash get_CTCF_signal_dhs_ctcf.sh /data/zusers/fankaili/ideas/dhs_ctcf/ 0
...
for i in {1..66}
do
    sample=`head -1 /data/zusers/fankaili/ideas/dhs_ctcf/ctcf_9sample_impute_result/ctcf_9sample_impute.chr1.state | awk -v n="$i" '{print $(n+4)}'`
    ...
    /data/zusers/fankaili/ideas/signal/rep1_signal_dhs_bins/${sample}_CTCF_dhs.tab \
