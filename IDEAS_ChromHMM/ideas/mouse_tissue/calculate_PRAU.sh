...
do
    ...
    echo $sample
    #
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$4]=$5;b[$4]=1}else{if(b[$4]){print $4,a[$4]}}}' ./state_bed_${prefix}/${sample}_state_sorted.bed ./state_ranked_PR/${sample}_CTCFpeak_pos.txt > tmp.${prefix}_pos.txt
    awk '{FS=OFS="\t"}{if(NR==FNR){a[$2]=$1}else{print $1,a[$2]}}' ./state_ranked_PR/${prefix}/${prefix}_state_CTCF_ranked.txt tmp.${prefix}_pos.txt > ./state_ranked_PR/${prefix}/${sample}_${prefix}_peak_PR_pos.txt
    #
    ...
    #
    prau=`Rscript ${scriptDir}calculate_PRAU.R ${workDir}state_ranked_PR/${prefix}/ ${sample}_${prefix}_peak_PR_pos.txt ${sample}_${prefix}_peak_PR_neg.txt`
    tmp=`awk '{print $2}' <<< $prau`
    echo -e ${sample}"\t"$tmp >> ./state_ranked_PR/${prefix}/${prefix}_PRAU.txt
done
