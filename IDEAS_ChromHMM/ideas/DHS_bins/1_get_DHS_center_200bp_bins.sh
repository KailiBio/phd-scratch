...
sort -k5,5n mm10-rOCRs_v3.txt | awk '{FS=OFS="\t"}{print $0,$3-$2}' > mm10-rOCRs_v3_sorted.txt
#
...
## manage small gaps
awk '{if(($3-$2)<100){print $0,$3-$2}}' mm10_rOCR_gap_v3.bed > temp2.txt
...
do
    a=`awk '{print $2}' <<< ${line}`
    b=`awk '{print $3}' <<< ${line}`
    num_a=`grep $'\t'${a}$'\t' mm10-rOCRs_v3_sorted_2.txt | wc -l`
    num_b=`grep $'\t'${b}$'\t' mm10-rOCRs_v3_sorted_2.txt | wc -l`
    if [ ${num_b} -ne 0 ]
    then
