...
# This one for CTCF dataset (25 exp used in definition)
# INPUT: bimodal values (signal/zscore matrix here)
# OURPUT: threshold

# EXP: Rscript classify_bimodal_EM.R /data/zusers/fankaili/ccre/tf/matrix/ CTCF
...
inFile = paste("hg19_ubi-rDHS_CTCF_2_zscore_matrix_ccreid.txt", sep="")
...
for(i in 1:ncol(data)){
  ...
  # density plot
  if(i==15 | i==20){
    hist(dat0, breaks=200, freq = FALSE, main=paste("CTCF:",biosample, sep=" "), xlab="zscore", col="grey")
  }else{
    hist(dat, breaks=200, freq = FALSE, main=paste("CTCF:",biosample, sep=" "), xlab="zscore", col="grey")
  }
  curve((myEM$lambda[1]*dnorm(x, myEM$mu[1], myEM$sigma[1])), col="green", lwd=2, add=TRUE)
  curve((myEM$lambda[2]*dnorm(x, myEM$mu[2], myEM$sigma[2])), col="yellow", lwd=2, add=TRUE)
  lines(c(1.64,1.64), c(0,2), col="blue", lwd=3, lty=2)
  lines(c(t,t), c(0,2), col="red", lwd=3, lty=2)
  if(i<6 | i==8 | i==9 | i==10 | i==16 | i==17 | i>21){
    legend("topleft", legend=paste("ubi-rDHS:",l1,l2,l3,sep="\n"), bty = "n")
  }else{
    legend("topright", legend=paste("ubi-rDHS:",l1,l2,l3,sep="\n"), bty = "n")
  }
}
dev.off()
