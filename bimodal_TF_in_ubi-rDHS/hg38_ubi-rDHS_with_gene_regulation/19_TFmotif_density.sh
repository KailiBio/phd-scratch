...
sed -i '$ d' rOCRs_motif_bed13.bed
sed -i '$ d' rOCRs_motif_bed13.bed
awk '{FS=OFS="\t"}{if(NR==FNR){a[$1":"$2"-"$3]=$4}else{print $1,$2,$3,a[$1":"$2"-"$3],$7,$8,$9,$10}}' /data/zusers/fankaili/ccre/hg38_ubi-rDHS/basic_annotation/rOCRs_overlap_hg38_v28_basic_TSS.bed rOCRs_motif_bed13.bed > rOCRs_motif_bed13_withID.bed
# get motif region
awk '{FS=OFS="\t"}{if(NR>1){print $1,$2+$5,$2+$6,$4,$7,$8}}' rOCRs_motif_bed13_withID.bed | sort -k1,1 -k2,2n | awk '{FS=OFS="\t"}{split($6,a,"(");print $1,$2,$3,$4,a[1]}' > motif_in_rOCRs.bed
